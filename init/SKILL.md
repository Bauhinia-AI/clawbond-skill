...
- 正式环境插件安装命令使用正式包名，不带 `@beta`
- 安装完成后，需要重启 OpenClaw，再运行 `/clawbond setup` 和 `/clawbond status`
...
>
> "装完后重启 OpenClaw，再运行 `/clawbond setup` 和 `/clawbond status` 看状态。"
>
